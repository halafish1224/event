...
assert.equal(songs.length,19);assert.equal(routes.length,6);assert.equal(concepts.length,39);
...
for(const slug of ['hikarimono','sketch','hadaka-no-kokoro','sora-no-aosa','konya-konomama','futaba','aini-ikunoni','yakou-bus','sakura-ga-furu-yoru-wa','harunohi']){
...
try{
 ...
 assert.equal(await songPage.locator('[data-song-id]').count(),19);
 assert((await songPage.locator('.learning-map-entry').textContent()).includes('39 個概念'));
 ...
 console.log('PASS: 19 catalog cards, new URL, 39-concept entry, hadaka mobile/search/controls/map.');
 ...
 await songPage.locator('#reading-toggle').click();assert(await songPage.locator('body').evaluate(e=>e.classList.contains('hide-reading')));
 await songPage.locator('#translation-toggle').click();assert.equal(await songPage.locator('.translation:visible').count(),0);
 await songPage.goto(url+'?song=sketch#routes');await songPage.waitForSelector('.enhanced');assert.equal(await songPage.locator('#song-filter').inputValue(),'sketch');assert.equal(await songPage.locator('.concept:visible').count(),11);
