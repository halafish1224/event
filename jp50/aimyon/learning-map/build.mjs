...
<html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive,nosnippet"><meta name="googlebot" content="noindex,nofollow"><meta name="referrer" content="strict-origin-when-cross-origin"><title>AIMYON｜跨歌曲學習地圖</title><meta name="description" content="${songs.length} 首歌曲、${routes.length} 條學習路線、${concepts.length} 個概念。用比較、回想與生活造句，串起日文學習。"><link rel="stylesheet" href="style.css"><script src="app.js" defer></script></head>
<body><a class="skip" href="#routes">跳至學習路線</a><main>
<header><a href="../songs/">← 回歌曲學習帳</a><p class="eyebrow">AIMYON NOTEBOOK / LEARNING MAP</p><h1>讓每一首歌，<br>接住上一個理解。</h1><p class="intro">${songs.length} 首歌・${routes.length} 條路線・${concepts.length} 個概念<br>從一句看懂，到換首歌也認得，再變成自己的表達。</p><nav class="top-nav" aria-label="頁內導覽"><a href="#today">今天學一點</a><a href="#routes">六條路線</a><a href="#song-list">全部歌曲</a><a href="#method">學習方法</a></nav></header>
...
<section id="routes"><h2>選一條今天想走的路</h2><p>這是概念順序，不是歌曲難度排名。可以自由跳讀；需要時沿「先學這個」回補基礎。</p><div class="route-grid">${routes.map((r,i)=>`<a class="route-card" href="#route-${r.id}"><span>ROUTE 0${i+1} · ${concepts.filter(c=>c.route===r.id).length} 個概念</span><strong>${r.title}</strong><small>${r.description}</small></a>`).join('')}</div></section>
<section class="panel js-only" aria-label="篩選概念"><label for="search">找語法、詞語或學習問題</label><input id="search" type="search" placeholder="例如：願望、ている、推測"><label for="song-filter">依歌曲篩選</label><select id="song-filter"><option value="">全部 ${songs.length} 首歌曲</option>${songs.map(s=>`<option value="${s.id}">${esc(s.title)}</option>`).join('')}</select><button type="button" id="clear-filter">清除篩選</button><p id="filter-count" role="status"></p><label><input type="checkbox" id="reading-toggle" checked> 顯示日文假名</label></section>
